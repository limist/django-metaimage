...
class BaseModel(models.Model):
    ...
    updater = models.ForeignKey(User, editable=False, null=True, related_name='%(class)s_updater')
...
class ImageSet(BaseModel):
    ...
    privacy = models.IntegerField(_('privacy'),choices=PRIVACY_CHOICES,default=1)
    tags = TagField()
...
class MetaImage(ImageModel, BaseModel):
    ...
    def save(self, *args, **kwargs):
        ...
        if getattr(self, 'source_url') and not getattr(self, 'image'):
            image_data_dct = fetch(self.source_url, max_size=MAX_REMOTE_IMAGE_SIZE) 
            if image_data_dct:
	...
	img_html = mark_safe('<img src="%s" height="%s" width="%s" alt="%s">' % (mimage_url, the_height, the_width, str(self.title)))
	if linked:
	    return mark_safe('<a href="%s" class="invisible">%s</a>' % (self.get_absolute_url(), img_html))
	else:
	    return img_html
